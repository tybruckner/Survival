...
```{r}
m6 = survreg(Surv(IP,Status)~Drop_MPH, dist = "weibull", data = statcast_SP)
summary(m6)
```
...
```{r}
m4_fs = survreg(Surv(Yrs,Status)~ IPY + SP, dist = "weibull", data = fullset)
summary(m4_fs)
```

```{r}
m5_fs = survreg(Surv(Yrs,Status)~ SP, dist = "weibull", data = fullset)
summary(m5_fs)
```

```{r}
m5_fs = survreg(Surv(IP,Status)~ SP, dist = "weibull", data = fullset)
summary(m5_fs)
```
